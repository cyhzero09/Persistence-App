...
class _AddCategorySheetState extends ConsumerState<AddCategorySheet> {
  ...
  void initState() {
    ...
    if (cat != null) {
      ...
      // 提醒设置存在打卡项目自身（reminderTime, HH:mm），直接读回开关与时间
      final rt = cat.reminderTime;
  ...
  Future<void> _save() async {
    ...
    // 打卡提醒就存在打卡项目自身：开关打开写时间，关闭写 null
    final reminderTimeStr = (_addReminder && _reminderTime != null)
    ...
    // 开关打开 → 按所选星期每周循环排定；关闭 → 取消已排定的通知。
    // 每周循环由系统闹钟自己滚下去，不需要用户每次手动重新开启。
    if (reminderTimeStr != null) {
      ...
      await NotificationService().cancelCheckInReminder(catId);
    }
